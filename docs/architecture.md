...
| `tibo.cjs` | 公共重置数据白名单、状态推导、请求合并和限频 | 推断个人额度到账、兑换重置券 |
...
AIHOT 请求放在已有加载器中，因为宿主 CSP 不允许页面连接该第三方域名。每次额度查询（含可见页面每分钟自动刷新、回到前台、账号切换和手动刷新）同时登记 Tibo 刷新意图，由加载器消费；不再独立轮询或提供单独重试按钮。支持 ETag、并发合并、五秒短时去重与 Retry-After；冷却中复用状态，失败等下一次联合刷新恢复。传入页面前只保留展示所需字段，原帖链接只接受指定来源。第三方请求故障不阻塞卡片与模型任务。
